class LOEditor{constructor(textarea,settings=null){this.id="lo-editor-"+crypto.randomUUID();this.container=$('<div id="'+this.id+'"></div>');this.toolbar=$('<lo-toolbar></lo-toolbar>');this.editor=$('<lo-editor contenteditable="true" spellcheck="false"></lo-editor>');this.crtMove=null;this.crtImg=null;this.textarea=$(textarea);this.plugins=[];this.stylesheet="";this.settings=settings;if(settings!=null&&typeof(settings.toolbar)!=="undefined"){this.buttonList=settings.toolbar}else{this.buttonList="heading bold italic code mark link blockquote pre left center right img separator lo-up lo-down lo-del lo-clean"}
this.textarea.hide();this.container.insertBefore(this.textarea);this.toolbar.appendTo(this.container);this.editor.appendTo(this.container);this.defaultPlugins();if(settings!=null&&typeof(settings.plugins)!=="undefined"){for(let i=0;i<settings.plugins.length;i++){this.addPlugin(settings.plugins[i])}}
this.addButtons();for(let i=0;i<this.plugins.length;i++){if(typeof(this.plugins[i].style)!=="undefined"){let the_stylesheet_lines=this.plugins[i].style.split("\n");for(let j=0;j<the_stylesheet_lines.length;j++){if(the_stylesheet_lines[j].trim().endsWith('{')&&!the_stylesheet_lines[j].trim().startsWith('@')){this.stylesheet+="#"+this.id+" "+the_stylesheet_lines[j].trim()+"\n"}else{this.stylesheet+=the_stylesheet_lines[j].trim()+"\n"}}}}
this.stylesheet=$('<style>',{type:'text/css',text:this.stylesheet}).appendTo(this.toolbar);this.load();for(let i=0;i<this.plugins.length;i++){if(typeof(this.plugins[i].events)!=="undefined"){this.plugins[i].events(this)}}
$(document).click(()=>{this.crtMove=null;this.crtImg=null;$(".lo-button-up").hide();$(".lo-button-down").hide();$(".lo-button-delete").hide()});$(this.editor).on('paste',(e)=>{e.preventDefault();e.stopPropagation();var text;var clp=(e.originalEvent||e).clipboardData;if(clp===undefined||clp===null){text=window.clipboardData.getData("text")||""}else{text=clp.getData('text/plain')||""}
text=this.sanitizeText(text);if(text.includes('<p>')){var crt=this.getCurrentParagraph();$(text).insertAfter(crt)}else{const selection=window.getSelection();if(!selection.rangeCount)return;const range=selection.getRangeAt(0);range.deleteContents();const textNode=document.createTextNode(text);range.insertNode(textNode);range.collapse(!1);selection.removeAllRanges();selection.addRange(range)}});$(this.editor).on('dragstart',(e)=>{e.originalEvent.dataTransfer.setData('text/html','<p>'+window.getSelection().toString()+'</p>')});$(this.editor).on('dragend',(e)=>{setTimeout(()=>{this.sanitizeEditor()},10)});$(this.editor).on('keydown',(e)=>{if(e.key==='Enter'||e.keyCode===13||e.key=='Backspace'||e.keyCode!==8||e.key==='Delete'||e.keyCode===46){setTimeout(()=>{this.sanitizeEditor()},100)}
if(e.keyCode==8||e.keyCode==46){if(this.crtImg!==null){$(this.crtImg).remove()}}});setInterval(()=>{this.save()},1000)}
getSelection(){const selection=window.getSelection();if(!selection.rangeCount||selection.isCollapsed)return[];const range=selection.getRangeAt(0);const result=[];const startNode=range.startContainer;const startOffset=range.startOffset;const endNode=range.endContainer;const endOffset=range.endOffset;const walker=document.createTreeWalker(range.commonAncestorContainer,NodeFilter.SHOW_TEXT,{acceptNode:function(node){if(range.intersectsNode(node)){return NodeFilter.FILTER_ACCEPT}
return NodeFilter.FILTER_REJECT}});let currentNode=walker.currentNode;if(currentNode.nodeType===Node.TEXT_NODE&&range.intersectsNode(currentNode)){processNode(currentNode)}
while(walker.nextNode()){processNode(walker.currentNode)}
function processNode(node){let from=0;let to=node.length;let selectedText=node.textContent.substring(from,to);if(node===startNode){from=startOffset}
if(node===endNode){to=endOffset}
result.push({node:node,parentElement:node.parentElement,textPart:selectedText,startOffset:from,endOffset:to})}
return result}
getCurrentParagraph(){const selection=window.getSelection();if(!selection.rangeCount)return;const range=selection.getRangeAt(0);var el=range.startContainer;while($(el).prop("tagName")!='P'&&$(el).hasParent()){el=$(el).parent()}
return el}
unwrap(el){if(el&&el.parentNode){while(el.firstChild){el.parentNode.insertBefore(el.firstChild,el)}
el.remove()}}
wrap(el,wrapper){var tag=null;if(typeof(wrapper)=="string"){tag=document.createElement(wrapper)}else{tag=wrapper}
if(el&&el.parentNode){el.parentNode.insertBefore(tag,el);tag.appendChild(el);return tag}}
clean(){$(this.editor).find('p').each(function(){if(this.nodeType===Node.ELEMENT_NODE){this.normalize()}else if(this.parentNode){this.parentNode.normalize()}})}
isSelectionEmpty(){const selection=window.getSelection();if(!selection.rangeCount)return;if(typeof selection.isCollapsed==='boolean')return selection.rangeCount===0||selection.isCollapsed;return!0}
toggleSelection(type){const parts=this.getSelection();if(!parts||parts.length===0)return;let unwrap=!1;for(let i=parts.length-1;i>=0;i--){const part=parts[i];const node=part.node;let targetNode=node;if(part.textPart.length===0)continue;if(node.parentElement.closest(type)){unwrap=!0;break}}
let returns=[];for(let i=parts.length-1;i>=0;i--){const part=parts[i];const node=part.node;let targetNode=node;if(part.textPart.length===0)continue;if(node.parentElement.closest(type)&&unwrap==!0){this.unwrap(node.parentElement.closest(type))}else if(unwrap==!1){if(part.endOffset<node.length){node.splitText(part.endOffset)}
if(part.startOffset>0){targetNode=node.splitText(part.startOffset)}
returns.push(this.wrap(targetNode,type))}}
this.clean();return returns}
toggleBlock(type){const selection=window.getSelection();if(!selection.rangeCount)return;const range=selection.getRangeAt(0);let disallowed=[];for(var i=0;i<this.plugins.length;i++){if(this.plugins[i].type=='block'){disallowed.push('lo-'+this.plugins[i].name)}}
if(this.isSelectionEmpty()){var p=this.getCurrentParagraph();if($(p).hasClass('lo-img')||$(p).hasClass('lo-separator'))return;if($(p).hasClass(type)){$(p).removeClass()}else{$(p).removeClass();$(p).addClass(type)}}else if($(range.commonAncestorContainer).parent().prop('tagName').toLowerCase()=='p'){if(disallowed.some(r=>$(range.commonAncestorContainer).parent().prop('class').split(' ').includes(r)))return;if($(range.commonAncestorContainer).parent().hasClass(type)){$(range.commonAncestorContainer).parent().removeClass()}else{$(range.commonAncestorContainer).parent().removeClass();$(range.commonAncestorContainer).parent().addClass(type)}}else{let ps=$(range.commonAncestorContainer).parent().find("p");let pss=Array.from(ps).filter(p=>{if(!selection.containsNode(p,!0))return!1;let pRange=document.createRange();pRange.selectNodeContents(p);return range.compareBoundaryPoints(Range.END_TO_START,pRange)<0&&range.compareBoundaryPoints(Range.START_TO_END,pRange)>0});pss.forEach((el)=>{if(disallowed.some(r=>$(el).prop('class').split(' ').includes(r)))return;if($(el).hasClass(type)){$(el).removeClass()}else{$(el).removeClass();$(el).addClass(type)}})}}
sanitizeEditor(){var allowed=[];var allowedin=['br'];const self=this;for(var i=0;i<this.plugins.length;i++){if(this.plugins[i].type=='block'||this.plugins[i].type=='textblock'){allowed.push('lo-'+this.plugins[i].name)}
if(typeof(this.plugins[i].allow)!=="undefined"){$.merge(allowedin,this.plugins[i].allow)}}
$(this.editor).find("> *").each(function(){if($(this).prop('tagName').toLowerCase()!='p'){$(this).replaceWith('<p>'+$(this).text()+'</p>')}});$(this.editor).find("p").each(function(){if(typeof($(this).prop('class'))!=="undefined"&&$(this).prop('class').trim()!=""&&!allowed.includes($(this).prop('class').toLowerCase().trim())){$(this).replaceWith('<p>'+$(this).text()+'</p>')}});$(this.editor).find("p *").each(function(){if(typeof($(this).prop('tagName'))!=="undefined"&&$(this).prop('tagName').trim()!=""&&!allowedin.includes($(this).prop('tagName').toLowerCase().trim())){$(this).replaceWith(document.createTextNode($(this).text()))}})}
del(){this.crtMove.remove()}
moveUp(){if(this.crtMove.previousElementSibling)
this.crtMove.parentNode.insertBefore(this.crtMove,this.crtMove.previousElementSibling);}
moveDown(){if(this.crtMove.nextElementSibling)
this.crtMove.parentNode.insertBefore(this.crtMove.nextElementSibling,this.crtMove);}
makeMovable(el){this.crtMove=el;$("button.lo-button-up").show();$("button.lo-button-down").show();$("button.lo-button-delete").show()}
sanitizeText(text){text=text.replace(/<[^>]*>/g,"");text=text.split("\n");if(text.length==1){return text[0]}else{for(var i=0;i<text.length;i++){if(text[i].trim()==""){text[i]=""}else{text[i]="<p>"+text[i]+"</p>"}}
return text.join('')}}
save(){var container=$(this.editor).clone();$(container).find('> *').each(function(){if($(this).prop('tagName').toLowerCase()!='p'){$(this).replaceWith('<p>'+$(this).html()+'</p>')}});this.plugins.forEach((plugin)=>{if(typeof(plugin.save)!=="undefined"){plugin.save(container,this)}});$(this.textarea).val($(container).html())}
load(){var container=$('<div>').html($(this.textarea).val());container.contents().filter(function(){return this.nodeType===3&&$.trim(this.nodeValue)}).replaceWith(function(){return $('<p>').html(this.nodeValue)});this.plugins.forEach((plugin)=>{if(typeof(plugin.load)!=="undefined"){plugin.load(container,this)}});container.find('> *').each(function(){if($(this).prop('tagName').toLowerCase()!='p'){$(this).replaceWith('<p>'+$(this).clone().wrap('<p>').parent().html()+'</p>')}});container.find('*').each(function(){$(this).removeAttr("style");$(this).removeAttr("width","");$(this).removeAttr("height","")});if(container.html()==""){container.html('<p>&nbsp;</p>')}
$(this.editor).append(container.html());this.sanitizeEditor()}
addButtons(){this.buttonList=this.buttonList.split(" ");this.buttonList.forEach((name)=>{this.plugins.forEach((plugin)=>{if(plugin.name==name){let el=$(plugin.button);$(this.toolbar).append(el);el.click((e)=>{plugin.action(e,this)})}})})}
addPlugin(plugin){this.plugins.push(plugin)}
defaultPlugins(){this.addPlugin({name:'heading',type:'textblock',style:`.lo-heading {
                font-size: 28px;
                font-weight: bold;
                margin-top: 30px;
            ...
            </button>`,action:()=>{this.toggleBlock('lo-heading')},save:(container)=>{$(container).find('.lo-heading').each(function(){$(this).replaceWith('<h2>'+$(this).html()+'</h2>')})},load:(container)=>{$(container).find('h2,h1,h3,h4').each(function(){$(this).replaceWith('<p class="lo-heading">'+$(this).html()+'</p>')})}});this.addPlugin({name:'bold',type:'child',allow:['lo-b'],style:`lo-b {
                font-weight: bold;
            ...
            </button>`,action:()=>{this.toggleSelection('lo-b')},save:(container)=>{$(container).find('lo-b').each(function(){$(this).replaceWith($('<b>'+$(this).html()+'</b>'))})},load:(container)=>{$(container).find('b,strong').each(function(){$(this).replaceWith('<lo-b>'+$(this).html()+'</lo-b>')})}});this.addPlugin({name:'italic',type:'child',style:`lo-i {
                font-style: italic;
            }`,allow:['lo-i'],button:`<button type="button" class="lo-button-italic"><svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24">
              <path d="M0 0h24v24H0z" fill="none" />
            ...
            </button>`,action:()=>{this.toggleSelection('lo-i')},save:(container)=>{$(container).find('lo-i').each(function(){$(this).replaceWith($('<i>'+$(this).html()+'</i>'))})},load:(container)=>{$(container).find('i,em').each(function(){$(this).replaceWith('<lo-i>'+$(this).html()+'</lo-i>')})}});this.addPlugin({name:'code',type:'child',allow:['lo-code'],style:`lo-code {
                background: #efefef;
                border-radius: 5px;
                padding: 2px 5px;
            ...
            </button>`,action:()=>{this.toggleSelection('lo-code')},save:(container)=>{$(container).find('lo-code').each(function(){$(this).replaceWith($('<code>'+$(this).html()+'</code>'))})},load:(container)=>{$(container).find('code').each(function(){$(this).replaceWith('<lo-code>'+$(this).html()+'</lo-code>')})}});this.addPlugin({name:'mark',type:'child',allow:['lo-mark'],style:`lo-mark {
                background: #F2E7D5;
                border-radius: 5px;
                padding: 2px 5px;
            ...
            </button>`,action:()=>{this.toggleSelection('lo-mark')},save:(container)=>{$(container).find('lo-mark').each(function(){$(this).replaceWith($('<mark>'+$(this).html()+'</mark>'))})},load:(container)=>{$(container).find('mark').each(function(){$(this).replaceWith('<lo-mark>'+$(this).html()+'</lo-mark>')})}});this.addPlugin({name:'link',type:'child',allow:['lo-link'],style:`lo-link {
                color: #990000;
                font-weight: bold;
                padding-bottom: 1px;
            ...
            </button>`,action:()=>{let link=this.toggleSelection('lo-link');if(link.length!=0){let url=prompt("Set an URL:");$(link).attr("href",url);$(link).on('click',function(e){if(e.ctrlKey||e.metaKey){window.open($(this).attr('href'))}});$(link).on('mouseover',function(e){$(this).attr("title","CTRL+Click to open: "+$(this).attr('href'))});$(link).on('mouseout',function(e){$(this).removeAttr("title")})}},save:(container)=>{$(container).find('lo-link').each(function(){$(this).replaceWith($('<a href="'+$(this).attr("href")+'">'+$(this).html()+'</a>'))})},load:(container)=>{$(container).find('a').each(function(){$(this).replaceWith('<lo-link href="'+$(this).attr('href')+'">'+$(this).html()+'</lo-link>')})},events:()=>{$(this.editor).find('lo-link').each(function(){$(this).on('click',function(e){if(e.ctrlKey||e.metaKey){window.open($(this).attr('href'))}});$(this).on('mouseover',function(e){$(this).attr("title","CTRL+Click to open: "+$(this).attr('href'))});$(this).on('mouseout',function(e){$(this).removeAttr("title")})})}});this.addPlugin({name:'blockquote',type:'textblock',style:`.lo-blockquote {
            ...
            </button>`,action:()=>{this.toggleBlock('lo-blockquote')},save:(container)=>{$(container).find('.lo-blockquote').each(function(){$(this).replaceWith('<blockquote>'+$(this).html()+'</blockquote>')})},load:(container)=>{$(container).find('blockquote').each(function(){$(this).replaceWith('<p class="lo-blockquote">'+$(this).html()+'</p>')})}});this.addPlugin({name:'pre',type:'textblock',style:`.lo-pre {
                background: #F3E8E8;
                padding: 10px 15px;
                color: #000;
            ...
            </button>`,action:()=>{this.toggleBlock('lo-pre')},save:(container)=>{$(container).find('.lo-pre').each(function(){$(this).replaceWith('<pre>'+$(this).html()+'</pre>')})},load:(container)=>{$(container).find('pre').each(function(){$(this).replaceWith('<p class="lo-pre">'+$(this).html()+'</p>')})}});this.addPlugin({name:'left',type:'textblock',style:`.lo-left {
                text-align: left;
            ...
            </button>`,action:()=>{this.toggleBlock('lo-left')},save:(container)=>{$(container).find('.lo-left').each(function(){$(this).replaceWith('<p class="lo-left">'+$(this).html()+'</p>')})},load:(container)=>{$(container).find('.lo-left').each(function(){$(this).replaceWith('<p class="lo-left">'+$(this).html()+'</p>')})}});this.addPlugin({name:'center',type:'textblock',style:`.lo-center {
                text-align: center;
            ...
            </button>`,action:()=>{this.toggleBlock('lo-center')},save:(container)=>{$(container).find('.lo-center').each(function(){$(this).replaceWith('<p class="lo-center">'+$(this).html()+'</p>')})},load:(container)=>{$(container).find('.lo-center').each(function(){$(this).replaceWith('<p class="lo-center">'+$(this).html()+'</p>')})}});this.addPlugin({name:'right',type:'textblock',style:`.lo-right {
                text-align: right;
            ...
            </button>`,action:()=>{this.toggleBlock('lo-right')},save:(container)=>{$(container).find('.lo-right').each(function(){$(this).replaceWith('<p class="lo-right">'+$(this).html()+'</p>')})},load:(container)=>{$(container).find('.lo-right').each(function(){$(this).replaceWith('<p class="lo-right">'+$(this).html()+'</p>')})}});this.addPlugin({name:'img',type:'block',allow:['img'],style:`.lo-img {
                border-radius: 10px;
                padding: 30px;
            ...
            </button>`,action:()=>{var crt=this.getCurrentParagraph();let el=$('<p class="lo-separator" contenteditable="false">&nbsp;</p>').insertAfter(crt);$(el).on("click",(e)=>{e.preventDefault();e.stopPropagation();this.makeMovable(el[0])})},save:(container)=>{$(container).find('.lo-separator').each(function(){$(this).replaceWith('<hr/>')})},load:(container)=>{$(container).find('hr').each(function(){$(this).replaceWith('<p class="lo-separator" contenteditable="false">&nbsp;</p>')})},events:()=>{$(this.editor).find(".lo-separator").on("click",(e)=>{e.preventDefault();e.stopPropagation();this.makeMovable(e.currentTarget)})}});this.addPlugin({name:'lo-up',type:'none',button:`<button type="button" class="lo-button-up" style="display: none;"><svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24">
              <path d="M0 0h24v24H0z" fill="none" />
            ...
            </button>`,action:(e)=>{e.preventDefault();e.stopPropagation();this.moveUp()}});this.addPlugin({name:'lo-down',type:'none',button:`<button type="button" class="lo-button-down" style="display: none;"><svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24">
              <path d="M0 0h24v24H0z" fill="none" />
            ...
            </svg></button>`,action:(e)=>{e.preventDefault();e.stopPropagation();this.moveDown()}});this.addPlugin({name:'lo-del',type:'none',button:`<button type="button" class="lo-button-delete" style="display: none;"><svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24">
              <path d="M0 0h24v24H0z" fill="none" />
            ...
            </svg></button>`,action:(e)=>{e.preventDefault();e.stopPropagation();this.del()}});this.addPlugin({name:'lo-clean',type:'none',button:`<button type="button" class="lo-button-clean"><svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24">
            	<path d="M0 0h24v24H0z" fill="none" />
            ...
            </button>`,action:(e,self)=>{var disallowed=[];for(var i=0;i<this.plugins.length;i++){if(this.plugins[i].type=='block'){disallowed.push('lo-'+this.plugins[i].name)}}
$(this.editor).find('> p').each(function(){if(!disallowed.includes($(this).prop('class').toLowerCase())&&$(this).text().trim()==''){$(this.remove())}})}})}}
